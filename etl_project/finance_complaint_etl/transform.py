from extract import ExtractOutput
...
class Transform:
    def __init__(self,extract_output:ExtractOutput,transform_config:TransformConfig):
        try:
    ...
    def start_transformation(self)->TransformOutput:
        try:
            ...
            spark_session=SparkSession.builder.master("local[*]").appName("ConsumerComplaintAnalysis").getOrCreate()

            for file_name in os.listdir(dir_name):
                file_path=".".join("file///",os.path.join(dir_name,file_name))
                ...
                tranform_file_path=os.path.join(self.transform_config.transform_dir,transform_file_name)
                df.write.parquet(tranform_file_path)
